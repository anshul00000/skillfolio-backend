...
cloudinary.config({ 
  cloud_name: 'don3api7h', 
  api_key: '346557983583949', 
  api_secret: 'qpGaS5xFn-OWk09Z3FGDsvpw5xU'
});
...
// import upload from "../middlewares/multer_mid_.js" ;
...
const API_KEY = 'c86afd0bc236d877172cd3ce49a16de2'; // Your ImgBB API Key

router.route('/asdf').get((req , res)=>{
  // res.send("hyy this is ibb ✅🙄");
  // res.json({ message: "hyy this is ibb ✅🙄" });
  res.render('qwer');
  ...
  const img_respons =  await cloudinary.uploader.upload(file , {folder : "skillfolio"})
  
  res.json({url : img_respons.secure_url , msg : img_respons});
  ...
  // const img_respons =   cloudinary.uploader.upload(img , {folder : "skillfolio"})
  // .then(result=>res.json({msg : result })).catch(err=>res.json({msg : err}));

  // res.json({msg : img_respons }) ;
...
router.route('/delete_img').get((req , res) => {
  ...
  // const image_url = "wwww.chatcpt/imsd/gege/geever/image.png" ;

const imgname = image.split('/')
const img_name =imgname[imgname.length - 1].split(".");


console.log(img_name[0]);

 const delete_res = await cloudinary.uploader.destroy(`skillfolio/${img_name[0]}`) ;

 res.json({msg : delete_res , img : image}) ;
     
  //   , (err, result) => {
  //   if (err) return res.status(400).json({ error: 'Image not found!' });
...
router.route('/').get(async (req, res) => {
  ...
  if (connect_db) {
    ...
    res.json({ message: "true hyy this is index ✅🙄" });
